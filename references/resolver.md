...
13. Near-black `#0B0C0B` + off-white `#F5F9F1` carry the system. **Mint `#BBFFA6` is the single accent.** Lime `#A2D860` and Frog `#3D5C17` support Mint; they don't replace it. The full canonical palette (10 tokens — do not introduce new brand hues for surfaces, charts, or components) also includes: Muted `#484848` (dark neutral support), Muted-Inverse `#979797` (secondary text, metadata, borders, chart labels), Surface 1 `#121312` (subtle lifted dark panel surface), Surface 2 `#181918` (nested or hover dark surface), Destructive `#E5484D` (destructive UI token only).
...
16. Inter is the required primary family for display, body, reading, and interface text (six imported weights: 300/400/500/600/700/900; Inter 18pt body via `--font-inter`, Inter 28pt display via `--font-inter-display` with tight -0.05em tracking). Red Hat Mono only for code, file paths, hex, exact values, and copy/paste blocks. Never decorative.
...
20. **Grain over Mint** (`.grain-mint`: `linear-gradient(180deg, #A2D860 5%, #BBFFA6 50%, #3D5C17 100%)` + grain overlay) **is the only sanctioned gradient.** Use ONLY for singular brand moments — hero shapes, section anchors, ad creative — and never as decorative edge strips, wallpaper, repeated fills, or general background texture. (Matches the canonical signatureSurface rule verbatim.)
